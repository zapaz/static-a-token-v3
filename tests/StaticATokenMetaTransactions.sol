...
contract StaticATokenMetaTransactions is BaseTest {
    ...
    function test_metaDepositATokenUnderlyingNoPermit() public {
        ...
        bytes32 digest = SigUtils.getTypedDepositHash(
            depositPermit, staticATokenLM.METADEPOSIT_TYPEHASH(), staticATokenLM.DOMAIN_SEPARATOR()
        );
    ...
    function test_metaDepositATokenUnderlying() public {
        ...
        bytes32 permitDigest = SigUtils.getTypedDataHash(permit, IERC20WithPermit(UNDERLYING).DOMAIN_SEPARATOR());

        (uint8 pV, bytes32 pR, bytes32 pS) = vm.sign(userPrivateKey, permitDigest);

        IStaticATokenLM.PermitParams memory permitParams =
            IStaticATokenLM.PermitParams(permit.owner, permit.spender, permit.value, permit.deadline, pV, pR, pS);
    ...
    function test_metaDepositAToken() public {
        ...
        bytes32 permitDigest = SigUtils.getTypedDataHash(permit, IERC20WithPermit(A_TOKEN).DOMAIN_SEPARATOR());
    ...
    function test_metaWithdraw() public {
        ...
        bytes32 digest = SigUtils.getTypedWithdrawHash(
            permit, staticATokenLM.METAWITHDRAWAL_TYPEHASH(), staticATokenLM.DOMAIN_SEPARATOR()
        );
        (uint8 v, bytes32 r, bytes32 s) = vm.sign(userPrivateKey, digest);
