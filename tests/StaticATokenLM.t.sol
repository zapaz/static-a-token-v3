...
contract StaticATokenLMTest is BaseTest {
    ...
    function test_getters() public {
        ...
        assertEq(address(staticATokenLM.INCENTIVES_CONTROLLER()), address(AToken(A_TOKEN).getIncentivesController()));
    }
    ...
    function test_maxDepositUnderlying_50kCap() public {
        ...
        assertEq(
            max,
            50_000 * (10 ** IERC20Metadata(UNDERLYING).decimals())
                - (
    ...
    function test_maxRedeemUnderlying_partAvailable() public {
        ...
        uint256 maxRedeemBefore = staticATokenLM.previewRedeem(staticATokenLM.maxRedeemUnderlying(address(user)));
        ...
        AaveV3Avalanche.POOL.borrow(UNDERLYING, underlyingBalanceBefore - (maxRedeemBefore / 2), 2, 0, borrowUser);

        uint256 maxRedeemAfter = staticATokenLM.previewRedeem(staticATokenLM.maxRedeemUnderlying(address(user)));
